...
\end{align}
The choice of functions reflect a predator specialized in hunting the most productive zones, and a heterogeneous productivity.
\begin{comment}
...
\begin{lemma}
  ...
  \end{equation}
\end{lemma}
A proof can be found in \citep[Proposition 2.8, p.96]{hadjisavvas2006handbook}. This allows us to state our main result on the behavorially modified Rosenzweig-MacArthur game.
\begin{proposition}
...
\begin{proof}
  ...
  To show that there exists a solution, start by noting that for all $w\in H^2$, $w\mapsto dU(w)$ is continuous, hence continuous on finite-dimensional subspaces. Then consider
  \begin{equation}
    \label{eq:du}
    \begin{split}
  ...
  \end{equation}
  where $G_1,G_2$ are uniformly bounded. With the constraint that $\norm{\sigma_c}_1 = 1, \norm{\sigma_p} = 1$ \Cref{eq:fin_exist} tends to infinity as $\norm{(\sigma_c,\sigma_p)}_2$ tends to infinity. Hence \Cref{eq:du} is only negative on a bounded subset of $K$, showing existence of a solution to the variational inequality by \Cref{thm:existence}.
...
\end{corollary}
\begin{proof}
Here we can directly use the theory we have developed.  The functions $f_1/N_c$ and $f_2/N_p$ are strictly monotone, and the set of fixed-points is uniformly bounded due to the metabolic terms and logistic terms, respectively. The Nash equilibrium exists and is unique for every $N_c, N_p$ , \Cref{prop:exist_unique_nash}. Therefore, by \Cref{thm:pop_game_exists_unique} there is a unique equilibrium for the population game.
\end{proof}
