...
\begin{proof}
  ...
  \end{equation}
  Introducing $\ket{x}$ as the functional defined from $x$, we calculate:
  \begin{equation}
    \label{eq:hess_mat}
    H(x) = (\nabla -dU)(x) =
    \begin{bmatrix}
      \frac{N_c}{K} - \frac{F_p N_c N_p \ket{\beta_p \sigma_p} \ket{\beta_p \sigma_p}}{(F_p + N_c \ip{\sigma_c}{\beta_p \sigma_p})^2} & \frac{F_p^2 N_p \ket{\beta_p \sigma_p}}{(F_p + N_c \ip{\sigma_c}{\beta_p \sigma_p})^2} \\
      \frac{\epsilon N_cF_p^2 (N_c \beta_p \ket{\beta_p \sigma_p} - \beta_p F_p)}{(F_p + N_c \ip{\sigma_c}{\beta_p \sigma_p})^3} & \frac{\epsilon N_c^2 F_p^2 \ket{\beta_p \sigma_c} \ket{\beta_p \sigma_c}}{(F_p + N_c \ip{\sigma_c}{\beta_p \sigma_p})^3}+c N_p \beta
    \end{bmatrix}
  \end{equation}
...
\begin{theorem}
  The population game \Cref{eq:dynamics} has a unique co-existence fixed point. Further, this fixed point is stable.
\end{theorem}
\begin{proof}
