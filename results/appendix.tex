...
\end{split}
\end{equation}
To handle \Cref{eq:tot_min_du}, we consider the individual terms. We start by considering the terms:
\begin{equation}
  \label{eq:unf_bnd}
  \begin{split}
  ...
  \end{split}
\end{equation}
We see both terms in \Cref{eq:unf_bnd} are uniformly bounded in $(\sigma_c,\sigma_p)$ over $P_{2,\mu}^2$, hence so is their difference $W_0(\sigma_c,\sigma_p)$.
Defining $C_1 = \frac{1}{K_0 + K \ess \sup \phi}$ and $C_2 = c \ess \inf \beta_p$ we can rewrite \Cref{eq:tot_min_du} as:
\begin{equation}
  \begin{split}
  ...
  \end{split}
\end{equation}
Since $\norm{\sigma_i}_1=1, i \in \{c,p\}$, all terms involving $\norm{\cdot}_1$ in \Cref{eq:tot_min_du} are uniformly bounded, and can be gathered with $W_0$ in a single uniformly bounded function $W$. Hence we end with:
\begin{equation}
